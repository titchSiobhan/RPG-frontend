	...
	return (
		<>
			...
			<div className={toggle ? 'inventory show' : 'inventory'}>
				<div
					className={toggle ? 'inventory-close show' : 'inventory-close'}
				...
				<div className="inventory-area">
					...
					{inventory.map((item, id) => (
						<div
							key={id}
							onClick={() => {
								(equipItem(item), toggleInventory());
							}}
						>
                            <div className="inventory-item">
							<p className="item-name">{item.name}</p>
                            {player.equip?.weapon?.name === item.name ? <p>Equipped</p> : null}
                            {player.equip?.armor?.name === item.name ? <p>Equipped</p> : null}
                            {player.equip?.shield?.name === item.name ? <p>Equipped</p> : null}
                            </div>
						</div>
					))}
				</div>
